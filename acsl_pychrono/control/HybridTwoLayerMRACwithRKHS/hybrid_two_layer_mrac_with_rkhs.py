...
from acsl_pychrono.control.HybridTwoLayerMRACwithRKHS.hybrid_two_layer_mrac_with_rkhs_gains import HybridTwoLayerMRACwithRKHSGains
...
from acsl_pychrono.control.HybridTwoLayerMRACwithRKHS.m_hybrid_two_layer_mrac_with_rkhs import M_HybridTwoLayerMRACwithRKHS
from acsl_pychrono.control.NonAdaptiveEBCI.m_nonadaptive_ebci import M_NonAdaptiveEBCI
from acsl_pychrono.control.HybridMRACwithRKHS.m_hybrid_mrac_with_rkhs import M_HybridMRACwithRKHS
...
class HybridTwoLayerMRACwithRKHS(BaseMRAC, Control):
  def __init__(self, gains: HybridTwoLayerMRACwithRKHSGains, ode_input: OdeInput, flight_params: FlightParams, timestep: float):
  ...
  def computeControlAlgorithm(self, ode_input: OdeInput):
    ...
    self.Theta_hat_rkhs_tran = self.y[135:159] # RKHS coefficient columns (translational)
    self.Theta_hat_rkhs_rot = self.y[159:183] # RKHS coefficient columns (rotational)
    ...
    if self.gains.use_hybrid:
      (self.e_tran_previous,
       self.e_rot_previous
      ) = M_HybridMRACwithRKHS.computePreviousTrajectoryTrackingErrors(self.e_tran, self.e_rot)
    ...
    self.mu_adaptive_mrac_tran = M_HybridTwoLayerMRACwithRKHS.computeControlLaw(
    ...
    self.mu_adaptive_tran = self.mu_adaptive_mrac_tran + self.mu_adaptive_rkhs_tran + self.mu_adaptive_ebci_tran
    ...
    self.Moment_adaptive_mrac = M_HybridTwoLayerMRACwithRKHS.computeControlLaw(
    ...
    self.Moment_adaptive = self.Moment_adaptive_mrac + self.Moment_adaptive_rkhs + self.Moment_adaptive_ebci
  ...
  def ode(self, t, y):
    ...
    self.dy[135:159] = self.Theta_hat_rkhs_tran_dot.reshape(24,1)
    self.dy[159:183] = self.Theta_hat_rkhs_rot_dot.reshape(24,1)
  ...
  def updateAdaptiveLawsOuterLoop(self):
    ...
    ) = M_HybridTwoLayerMRACwithRKHS.computeAllRobustAdaptiveLaws(
  ...
  def computeRKHSAdaptiveLaw(self, Gamma_rkhs, Phi_rkhs, eTranspose_P_B, dead_zone_value,
    ...
    classic_term = Phi_rkhs * eTranspose_P_B
    if self.gains.use_e_modification:
      classic_term = classic_term - sigma_rkhs * eTranspose_P_B_norm * Theta_hat_rkhs
    return Gamma_rkhs * modulation_factor * classic_term
  ...
  def computePostIntegrationAlgorithm(self):
    if self.gains.use_hybrid:
      ...
      ) = M_HybridMRACwithRKHS.runHybridStep(
        self.e_tran,
        self.e_tran_previous,
        self.gains.P_tran,
        self.gains.Q_tran,
