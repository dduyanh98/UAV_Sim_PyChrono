...
from acsl_pychrono.rkhs import RKHSRegressorConfig

class HybridTwoLayerMRACwithRKHSGains(BaseMRACGains):
  def __init__(self, flight_params: FlightParams):
    ...
    self.rkhs_feature_size = 8
    self.rkhs_theta_size = self.rkhs_feature_size * 3
    self.number_of_states = 135 + 2 * self.rkhs_theta_size
    ...
    # RKHS adaptive coefficient vectors. Octree mode uses 8 active kernel centers.
    self.Gamma_rkhs_tran = np.matrix(np.eye(self.rkhs_feature_size))
    self.Gamma_rkhs_rot = np.matrix(np.eye(self.rkhs_feature_size))
    self.sigma_rkhs_tran = self.sigma_Theta_tran
    self.sigma_rkhs_rot = self.sigma_Theta_rot
    ...
    # Projection bounds for the additional RKHS coefficient columns.
    self.x_e_rkhs_tran = np.zeros((self.rkhs_theta_size, 1))
    self.x_e_rkhs_rot = np.zeros((self.rkhs_theta_size, 1))
    self.S_diagonal_rkhs_tran = np.ones((self.rkhs_theta_size, 1))
    self.S_diagonal_rkhs_rot = np.ones((self.rkhs_theta_size, 1))
    self.alpha_rkhs_tran = self.alpha_Theta_tran
    self.alpha_rkhs_rot = self.alpha_Theta_rot
    ...
    self.S_rkhs_tran = ProjectionOperator.generateEllipsoidMatrixFromDiagonal(self.S_diagonal_rkhs_tran.flatten())
    self.S_rkhs_rot = ProjectionOperator.generateEllipsoidMatrixFromDiagonal(self.S_diagonal_rkhs_rot.flatten())
    ...
    self.epsilon_rkhs_tran = ProjectionOperator.computeEpsilonFromAlpha(self.alpha_rkhs_tran)
    self.epsilon_rkhs_rot = ProjectionOperator.computeEpsilonFromAlpha(self.alpha_rkhs_rot)
    ...
    self.rkhs_config = RKHSRegressorConfig()
    
    print(f"[INFO] Successfully loaded HybridTwoLayerMRACwithRKHS Gains")
