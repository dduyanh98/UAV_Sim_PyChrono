...
from acsl_pychrono.control.HybridTwoLayerMRACwithRKHS.hybrid_two_layer_mrac_with_rkhs import HybridTwoLayerMRACwithRKHS
from acsl_pychrono.rkhs.workspace_logger import RKHSWorkspaceLogger

class HybridTwoLayerMRACwithRKHSLogger:
  def __init__(self, gains: HybridTwoLayerMRACwithRKHSGains) -> None:
    self.gains = gains
    self.data_list = []
    self.rkhs_logger = RKHSWorkspaceLogger()
    # Length of the array vector that will be exported 
    self.size_DATA = 222

  def collectData(self, controller: HybridTwoLayerMRACwithRKHS, simulation_time: float):
    ...
    self.rkhs_logger.collectData(controller, simulation_time)
  ...
  def toDictionary(self):
    ...
    log_dict["rkhs"] = self.rkhs_logger.toDictionary()
